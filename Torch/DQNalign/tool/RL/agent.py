...
class Agent():
    def __init__(self, FLAGS, istrain, game_env, model, seq1 = [], seq2 = [], ismeta = False):
        ...
        if FLAGS.model_name == 'C51':
            self.mainQN : torch.nn.Module = C51(self.param.h_size, self.env, 'main', 51, 3)
            self.targetQN : torch.nn.Module = C51(self.param.h_size, self.env, 'target', 51, 3)
            self.mainQN = self.mainQN.to(self.device)
            self.targetQN = self.targetQN.to(self.device)
            self.optimizer = torch.optim.Adam(params=self.mainQN.parameters(), lr=5e-4)
    ...
    def train(self):
        ...
        if self.FLAGS.model_name == 'C51':
            ...
            for i in range(self.param.batch_size):
                if done[i]:
                    ...
                    for j in range(self.num_atoms):
                        Tz = min(self.vmax, max(self.vmin, rewards[i] + self.param.y * self.z[j]))
                        bj = (Tz - self.vmin)/self.delta_z
                        m_l, m_u = np.floor(bj), np.ceil(bj)
            ...
            self.soft_target_update(self.param.tau)
    ...
    def compute_loss_c51(self, states : torch.Tensor, probs: torch.Tensor, tree_idx=None) -> torch.Tensor:
        ...
        if USE_PER:
            ...
            to_add = np.mean(np.array(to_add), axis=1)
