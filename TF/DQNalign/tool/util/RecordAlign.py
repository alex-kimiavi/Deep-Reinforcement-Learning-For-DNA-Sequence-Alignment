...
class record_align():
    ...
    def print_identity(self, file, seq1, seq2):
        self.postprocessing()
        ...
        for i in range(int(np.ceil(len(self.Qtemp)/60))):
            ...
            file.write(self.Stemp[60*i:60*i+60]+"\n")
            file.write("\n")
        with open('identity/' + str(self.test_type) + '/HEV_identity_tests.txt', 'a') as f:
            matches = 0 
            if len(seq1) <= len(seq2):
                for i in range(len(seq1)):
                    matches += 1 if seq1[i] == seq2[i] else 0
            else:
                for i in range(len(seq2)):
                    matches += 1 if seq2[i] == seq2[i] else 0 
            to_add = str(matches/len(seq1)) + ' ' + str(matches/len(seq2)) + ' ' +  str(self.stemp/len(seq1)) + ' ' + str(self.stemp/len(seq2)) + '\n'
            f.write(to_add)
